...
/*The following function is activated by the confirm time button. It requests the products from the API, and clones the modelContainer for each product in the DB.
 ...
 */
//The global variable storedProducts stores the product objects that are received from the database
...
function confirmTime() {
    ...
    //Checks that all date/time fields have been filled out.
    if (rentDayValue != "00" && rentMonthValue != "00" && rentYearValue != "00" && rentTimeValue != "00") {
        ...
        //Locks the date/time selection elements so that they can't be changed once a date/time has been confirmed.
        ...
        fetch('http://localhost:3000/orderPage/products', {
            ...
            .then(json => {
                ...
                //Checks if the products have already been generated on the page. This is to avoid JS creating duplicates of the same products if the button is pressed repeatedly
                if (storedProducts.length !== json.length) {
                    ...
                    for (let i = 0; i < json.length; i++) {
                        //Creates a new Product object from the received information and pushes it to the storedProducts array
                        ...
                        if (json[i].quantity !== 0) {
                            //The shown products array only contains the objects that are shown on the page
        ...
        //If the user has not filled out all the date/time fields, an error is shown:
    } else {
        alert("Udfyld venligst alle felter.");
    }
}

/*The following function is activated when the user changes the amount of selected products. It has the following functionality:
...
function calculatePrice() {
    ...
    //Hides the basket if the basket is empty
    if (finalPrice === 0) {
...
//The storeOrder function collects the order information and sends it to the API
...
function storeOrder() {
    //The Products array is created. It will contain all the selected products.
    var products = [];
