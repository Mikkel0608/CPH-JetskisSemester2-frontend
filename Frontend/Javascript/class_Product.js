export default class Product {
    ...
    generateProduct(clonedElement, i) {
        ...
        //The following lines insert the fetched product information onto each clone
        ...
        //Inserts maximum amount of available products. It cycles through the product quantity and creates the amount of select options needed.
    ...
    generateBasketProduct(clonedElement, modelElement, x) {
        if (document.getElementById(modelElement + x) !== null) {
            //Saves the select element of product number x in the loop
        console.log(modelElement + [x]);
            ...
            //If statement checks if there are more than 0 selected of the product, and that the product has not been generated in the basket already
            if (selectElement2.options[selectElement2.selectedIndex].value > 0 && document.getElementById(clonedElement + x) == null) {
                ...
                //Inserts each product clone onto the "productContainer" node
                document.getElementById(clonedElement).parentElement.appendChild(clone);
                //Inserts all the product information for the corresponding product stored in the storedProducts array
                ...
                //If the product element already exists in the basket, and the "select" element is changed to "0", it hides the element from the basket:
                ...
                //If the product element already exists, but has been hidden previously, it makes the div visible and updates the div with correct amount/price:
            } else if (selectElement2.options[selectElement2.selectedIndex].value > 0 && document.getElementById(clonedElement + x) != null) {
